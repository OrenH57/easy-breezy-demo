...
const authed = (req) => sessions.has(cookies(req).eb_session);
...
const mime = { '.css': 'text/css; charset=utf-8', '.js': 'text/javascript; charset=utf-8', '.jpg': 'image/jpeg', '.jpeg': 'image/jpeg', '.png': 'image/png', '.svg': 'image/svg+xml', '.ico': 'image/x-icon', '.webp': 'image/webp', '.woff2': 'font/woff2' };
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (req.method === 'POST' && url.pathname === '/api/admin/login') { const input = await body(req); if (!crypto.timingSafeEqual(passwordHash(clean(input.password)), passwordHash(config.adminPassword))) return json(res, 401, { error: 'Incorrect password.' }); const token = id(); sessions.set(token, Date.now()); res.writeHead(200, { 'Content-Type': 'application/json', 'Set-Cookie': `eb_session=${token}; HttpOnly; SameSite=Lax; Path=/; Max-Age=28800` }); return res.end('{"ok":true}'); }
    if (req.method === 'POST' && url.pathname === '/api/admin/logout') { sessions.delete(cookies(req).eb_session); res.writeHead(200, { 'Content-Type': 'application/json', 'Set-Cookie': 'eb_session=; HttpOnly; SameSite=Lax; Path=/; Max-Age=0' }); return res.end('{"ok":true}'); }
...
server.listen(process.env.PORT || 4173, process.env.HOST || '0.0.0.0', () => { console.log(`Easy Breezy running on port ${process.env.PORT || 4173}`); runDueReminderSweep(); setInterval(runDueReminderSweep, 24 * 60 * 60 * 1000); });
