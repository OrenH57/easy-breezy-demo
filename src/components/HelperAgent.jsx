...
export function HelperAgent() {
  ...
  return <aside className={`eb-helper${isAvailabilityVisible ? ' eb-helper-paused' : ''}`} aria-label="Easy Breezy service guide">
    ...
    </div> : <button className="helper-launcher helper-avatar-launcher" type="button" aria-label={canContinueBooking ? 'Continue your booking' : 'Ask Maya to help choose a service'} onClick={canContinueBooking ? continueBooking : () => setIsOpen(true)}><span className={`helper-sign${canContinueBooking ? ' helper-sign-continue' : ''}`}>{canContinueBooking ? 'Continue here?' : 'Hi, I’m Maya — I’m here to help.'}</span><span className="helper-avatar-frame"><img src={helperAgent} alt="Maya from Easy Breezy" /></span></button>}
  </aside>;
}
