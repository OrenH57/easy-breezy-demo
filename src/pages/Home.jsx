...
import { BadgeCheck, CalendarDays, CircleDollarSign, MapPin } from 'lucide-react';
...
const trust = [[MapPin, 'Local Maryland & DC team'], [CircleDollarSign, 'Free estimates'], [BadgeCheck, 'Respectful in-home service'], [CalendarDays, 'Easy online booking']];
...
export default function Home() { return <>
  <section className="original-hero upgraded-hero">
    <div className="original-hero-copy"><h1>A fresher home<br />starts with <span>clean air ducts.</span></h1><p className="lede">Professional air duct, dryer vent, and chimney service for homes that deserve clean, comfortable air.</p><div className="hero-points"><span>⚡ Clear next steps.</span><span>♡ Respectful service.</span><span>◉ Local coordination.</span></div><div className="hero-actions"><Link className="button original-primary" to="/booking">Get Free Quote <span>→</span></Link><Link className="hero-secondary" to="/services">Explore services</Link></div></div>
  ...
  <section className="trust-rail" aria-label="Easy Breezy service promises">{trust.map(([Icon, title]) => <article key={title}><Icon aria-hidden="true" /><strong>{title}</strong></article>)}</section>
